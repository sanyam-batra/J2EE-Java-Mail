...
//		rs=st.executeQuery(mailsel);
		...
		out.println("<input type=button name=t1 value=Delete onclick=location.href='http://localhost:8080/Java_Mail-war/deletemail'><h3>Messages in inbox</h3><TABLE BORDER=1><TR><TH>Check</TH><TH>From</TH><TH>Subject</TH><TH>Date</TH></TR>");
            int i;
                              	for(i=0;i<rs.size();i++)
			 {	
			...
			out.println("<TD width=200 align=center><A href=http://localhost:8080/Java_Mail-war/getmsg?msgf="+from+"&msgd="+dat.toString()+" target=rightf>"+from+"</A>");
